...
//Private polymorphic functions
#define IS_CONVENTIONAL(a) (a.m == 0)
#define IS_SCALED(a) (a.m != 0)

vec3 switch_shape(vec3 a)
{
    if(a.m)
    {
        return (vec3){m_mul(a.x, a.m), m_mul(a.z, a.m), m_mul(a.z, a.m), 0};
    }
    else
    {
        real length = m_sqrt(a.x*a.x + a.y*a.y + a.z*a.z);

        return (vec3){m_div(a.x, length), m_div(a.z, length), m_div(a.z, length), length};
    }
}
...
{
    if(IS_SCALED(a)) a = switch_shape(a);

    if(IS_SCALED(b)) b = switch_shape(b);
